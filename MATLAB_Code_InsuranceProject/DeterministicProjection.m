%% POINT c: Deterministic case
% The function BOF is called everytime with the flag variable of the asset
% simlation equal to 3 in order to simulate the assets in a deterministic
% way.
clc, clear, close all;
format long
seed = 10;
...
base = BOF(Eq0,Pr0,M,T,sigmaEq,sigmaPr,fwd_rates,...
                  RD,COMM,lapse,q,x,Pen,C0,inflation,expenses,F0,discounts,3);
...
pr = BOF(Eq0,Pr0*(1-P_sp),M,T,sigmaEq,sigmaPr,fwd_rates,...
                  ...
                  expenses,Eq0+Pr0*(1-P_sp),discounts,3)
% Compute the SCR:
...
SCR_mkt = sqrt([SCR_irdown, SCR_s1, SCR_sp]*CorrM*[SCR_irdown, SCR_s1,...
                SCR_sp]');
